...
public class EcommSignUpFirstNameValidations {
	...
	// validate that the First Name entered is less than or equal to 50 alphabetical characters in length. 
  ...
  public void enterValidLength(String name) {
	  ...
	  signup.firstNameField.sendKeys(name);
	  signup.signUpBtn.click();
	  Assert.assertFalse(signup.firstNameErrorMsg.isDisplayed());
  }
  
  @Test(description="validating the first name field DOESN'T accept values that are > 50 characters in length")
  public void enterInvalidLength() {
	//5.Enter a value that is 51 alphabetical characters in length, and verify you DO get an error message. 
	  signup.firstNameField.sendKeys("aaaklsdajdkhsdjbJJkkiKJDHFKSDFJKGBJDFGBFGVNBDFkjhhu");
	  signup.signUpBtn.click();
	  Assert.assertTrue(signup.FirstNameCharsLengthError.isDisplayed());
  }
...
//validate that the First Name entered does not contain non-alphabetical characters. 
//If the entered First Name entered contains non-alphabetical characters then I should see the following error message:
//”The firstName format is invalid.”
  ...
  public void enterInvalidFormat(String name) {
	  ...
	  signup.firstNameField.sendKeys(name);
	  signup.signUpBtn.click();
	  Assert.assertTrue(signup.FirstNameFormatError.isDisplayed());
  }
